...
/****** Object:  StoredProcedure [DBBackup].[DatabaseBackup]    Script Date: 7/16/2020 8:23:51 AM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [DBBackup].[DatabaseBackup]
...
BEGIN
  ...
  IF NOT EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'P' AND schemas.[name] = 'Utility' AND objects.[name] = 'usp_command_execute')
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The stored procedure usp_command_execute is missing. Download https://ola.hallengren.com/scripts/usp_command_execute.sql.', 16, 1
  END

  IF EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'P' AND schemas.[name] = 'Utility' AND objects.[name] = 'usp_command_execute' AND OBJECT_DEFINITION(objects.[object_id]) NOT LIKE '%@DatabaseContext%')
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The stored procedure usp_command_execute needs to be updated. Download https://ola.hallengren.com/scripts/usp_command_execute.sql.', 16, 1
  END
  ...
  IF @DatabasesInParallel = 'Y' AND NOT EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'U' AND schemas.[name] = 'DBBackup' AND objects.[name] = 'Queue')
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
    SELECT 'The table Queue is missing. Download https://ola.hallengren.com/scripts/Queue.sql.', 16, 1
  END

  IF @DatabasesInParallel = 'Y' AND NOT EXISTS (SELECT * FROM sys.objects objects INNER JOIN sys.schemas schemas ON objects.[schema_id] = schemas.[schema_id] WHERE objects.[type] = 'U' AND schemas.[name] = 'DBBackup' AND objects.[name] = 'QueueDatabase')
  BEGIN
    INSERT INTO @Errors ([Message], Severity, [State])
  ...
  BEGIN
    ...
    BEGIN TRY
      ...
      BEGIN
        ...
        SELECT @QueueID = QueueID
        FROM DBBackup.[Queue] WITH (UPDLOCK, HOLDLOCK)
        WHERE SchemaName = @SchemaName
        AND ObjectName = @ObjectName
        AND [Parameters] = @Parameters
        ...
        IF @QueueID IS NULL
        BEGIN
          INSERT INTO DBBackup.[Queue] (SchemaName, ObjectName, [Parameters])
      ...
      AND NOT EXISTS (SELECT *
                      FROM DBBackup.QueueDatabase QueueDatabase
      ...
      IF @@ROWCOUNT = 1
      BEGIN
        INSERT INTO DBBackup.QueueDatabase (QueueID, DatabaseName)
        SELECT @QueueID AS QueueID,
               DatabaseName
        FROM @tmpDatabases tmpDatabases
        WHERE Selected = 1
        AND NOT EXISTS (SELECT * FROM DBBackup.QueueDatabase WHERE DatabaseName = tmpDatabases.DatabaseName AND QueueID = @QueueID)
    ...
    BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          EXECUTE @CurrentCommandOutput = Utility.usp_command_execute @DatabaseContext = @CurrentDatabaseContext, @Command = @CurrentCommand, @CommandType = @CurrentCommandType, @Mode = 1, @DatabaseName = @CurrentDatabaseName, @LogToTable = @LogToTable, @Execute = @Execute
          SET @Error = @@ERROR
      ...
      BEGIN
        ...
        BEGIN
          ...
          SELECT @CurrentCommand = CASE
          WHEN @CurrentBackupType IN('DIFF','FULL') THEN 'DECLARE @ReturnCode int EXECUTE @ReturnCode = DBBackup.xp_backup_database @database = N''' + REPLACE(@CurrentDatabaseName,'''','''''') + ''''
          WHEN @CurrentBackupType = 'LOG' THEN 'DECLARE @ReturnCode int EXECUTE @ReturnCode = DBBackup.xp_backup_log @database = N''' + REPLACE(@CurrentDatabaseName,'''','''''') + ''''
          END
        ...
        BEGIN
          ...
          SET @CurrentCommand = 'DECLARE @ReturnCode int EXECUTE @ReturnCode = DBBackup.sqlbackup N''-SQL "' + REPLACE(@CurrentCommand,'''','''''') + '"''' + ' IF @ReturnCode <> 0 RAISERROR(''Error performing SQLBackup backup.'', 16, 1)'
        END
        ...
        BEGIN
          ...
          SET @CurrentCommand = 'DECLARE @ReturnCode int EXECUTE @ReturnCode = DBBackup.xp_ss_backup @database = N''' + REPLACE(@CurrentDatabaseName,'''','''''') + ''''
        ...
        BEGIN
          ...
          SET @CurrentCommand = 'DECLARE @ReturnCode int EXECUTE @ReturnCode = DBBackup.emc_run_backup '''
      ...
      BEGIN
        ...
        BEGIN
          ...
          BEGIN
            ...
            SET @CurrentCommand = 'DECLARE @ReturnCode int EXECUTE @ReturnCode = DBBackup.xp_restore_verifyonly'
          ...
          BEGIN
            ...
            SET @CurrentCommand = 'DECLARE @ReturnCode int EXECUTE @ReturnCode = DBBackup.sqlbackup N''-SQL "' + REPLACE(@CurrentCommand,'''','''''') + '"''' + ' IF @ReturnCode <> 0 RAISERROR(''Error verifying SQLBackup backup.'', 16, 1)'
          END
          ...
          BEGIN
            ...
            SET @CurrentCommand = 'DECLARE @ReturnCode int EXECUTE @ReturnCode = DBBackup.xp_ss_verify @database = N''' + REPLACE(@CurrentDatabaseName,'''','''''') + ''''
